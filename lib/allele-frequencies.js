...
var AlleleFreq = function (samples, range, refSeq, vars) {
    ...
    return {
         ...
         * It will iterate the sequence once and calculate the base frequencies for each position.
         */
        calculateFrequencies: function () {
            // Check if any variants have been parsed
            if (Object.getOwnPropertyNames(variants).length === 0) {
            ...
            for ( var i = start; i <= end; i++ ) {
                ...
                if (!variants[i]) {
                    /*
                     If there is no variant at this position,
                     set it to homozygous -> freq to 2 * number of samples
                     */
                    ...
                    var refBase = refs[i - start];
                    count[refBase] = 2 * (samples.length - numOfVars);
                    //iterate over all variants found at this position
                    variants[i].forEach(function (v) {
                        // Increase count of affected bases
                        count[v.alt] += v.nalt;
                        count[v.ref] += v.nref;
                    });
                }
                // The total number of alleles is always the same
                var sum =  2 * samples.length;
                // Normalize frequencies using to total allele count for each base
                for (var base in count) {
                    if (count.hasOwnProperty(base)) {
                        count[base] = +(Math.round((count[base] / sum) + "e+2")  + "e-2"); // JS is weird with floats
                    }
                }
                // Shift position for frequencies to start at 1 instead of start.
                frequencies[i + 1 - start] = count;
            }
        },

        /*
         Return frequencies as object with position as keys
         */
        ...
        /*
             Write positions to file at given path
         */
